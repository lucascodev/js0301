...
import { addDays, differenceInSeconds, endOfWeek, startOfWeek } from "date-fns";
  ...
  const render = () => {
    ...
    while (differenceInSeconds(ultimoCalendario, diaCorrente) > 0) {
      ...
      li.addEventListener("click", (e: Event) => {
        ...
        const myLi = e.target as HTMLLabelElement;

        myLi.classList.add("selected");
        const input = document.querySelector("[name=schedule_at]") as HTMLInputElement;
        input.value = myLi.dataset.schedule ?? "";
